...
export async function main(ns) {
	...
	while (true) {
		...
		if (security >= SECURITY_MIN + 3) {
			...
			let rest = start_threads(ns, WEAKEN_SCRIPT, target, threads, 0);
		...
		if (money < MONEY_MAX * 0.9) {
			...
			let rest = start_threads(ns, GROW_SCRIPT, target, threads, 0);
		...
		let rest = start_threads(ns, HACK_SCRIPT, target, threads, 0);
